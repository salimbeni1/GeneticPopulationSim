...
#because i m using the same plt for all the graphs as it isnt static : i m removing the warnings
import warnings
import matplotlib.cbook
warnings.filterwarnings("ignore",category=matplotlib.cbook.mplDeprecation)

#plt.warnings.warn(message, mplDeprecation, stacklevel=1)
...
#positionGraph = 221
#positionGraph += 2
#ax1 = fig.add_subplot(positionGraph)
...
#plt.title("allele frequency in Populations")	
cnt3 = 0
#positionGraph = 810
cnt4 = 1
for caselle in matrix[1:]:
    for caselline in caselle:
	positionGraph = 10+(len(matrix[1:])*100)
	positionGraph += cnt4
	ax1 = fig.add_subplot(positionGraph)
	#ax1.clear()
	ax1.set_ylim([0, 1])
	#ax1.set_xlabel("generation")
	ax1.set_ylabel("P_"+str(cnt4))
        ax1.plot(matrix[0][0],caselline)
	if(cnt4 != len(matrix[1:])):
		ax1.set_xticks([], [])
    cnt4 += 1
